...
from MarkMusic import YouTube, app
from MarkMusic.core.call import Mark
from MarkMusic.misc import SUDOERS, db
from MarkMusic.utils.database import (is_active_chat,
...
from MarkMusic.utils.formatters import seconds_to_min
from MarkMusic.utils.inline.play import (panel_markup_1,
                                          panel_markup_2,
                                          panel_markup_3,
                                          stream_markup,
                                          telegram_markup)
from MarkMusic.utils.stream.autoclear import auto_clean
from MarkMusic.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Mark.pause_stream(chat_id)
        ...
        await Mark.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
            _["admin_4"].format(mention)
        )
    elif command == "Stop" or command == "End":
        await CallbackQuery.answer()
        await Mark.stop_stream(chat_id)
        ...
        await Mark.mute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await Mark.unmute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Mark.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Mark.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await Mark.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
